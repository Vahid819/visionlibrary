...
export async function GET() {
  ...
  try {
    const students = await StudentModel.find({ createdBy: session.user.id })
      .sort({ createdAt: -1 })
      .lean();
  ...
  try {
    ...
    if (student.seat) {
      if (!seatAvailable) {
        return NextResponse.json(
          {
            message: `Seat ${body.seat} is already occupied or does not exist.`,
          },
          { status: 400 },
        );
      }
    ...
    return NextResponse.json({
      success: true,
      message: "Student deactivated and seat freed",
    });
  } catch (error) {
    return NextResponse.json(
